...
public class Fantome extends MazeObject implements Personnage, PacmanObserver {
   ...
   @Override
    ...
    public void notifyPacman(PacmanEvent event) {
    	...
    	else if(event.getState() == PacmanStates.Normal) {
    		NormalFantomeState normalFantomeState = new NormalFantomeState(this);
    		normalFantomeState.handle();
    	}
    	if(event.getCoordinate().getX() == fantomePosition.getX() && fantomePosition.getY() == event.getCoordinate().getY()){
            if(game.getPacman().getState().getPacmanState() != PacmanStates.Invisible) {
            	game.CollisionGhostPacman(this);
